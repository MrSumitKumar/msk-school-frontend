...
import {
    ...
    FileText, LogOut, Bell, ChevronDown, GraduationCap, Menu, X
} from 'lucide-react';
...
const navItems = [
    ...
    { to: '/teacher/exams', icon: FileText, label: 'Exams & Results' },
];
            ...
            <aside className={`sidebar ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
                <div style={{ padding: '24px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #10B981, #059669)' }}>
                            <GraduationCap size={20} color="white" />
                        </div>
                        <div>
                            <div style={{ fontWeight: 700, fontSize: 16 }}>{user?.school_name || 'SchoolSaaS'}</div>
                            <div style={{ color: 'var(--text-secondary)', fontSize: 11 }}>Teacher Portal</div>
                        </div>
                    </div>
                ...
                <nav style={{ padding: '12px 0', flex: 1 }}>
                    <div style={{ padding: '8px 16px 4px', color: 'var(--text-secondary)', fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase' }}>Teacher Menu</div>
                ...
                <div style={{ padding: '16px', borderTop: '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '8px', borderRadius: 10 }} onClick={() => setShowUserMenu(!showUserMenu)}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: 'white', background: 'linear-gradient(135deg, #10B981, #059669)' }}>
            ...
            <main className="main-content" style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div>
                            <h2 style={{ fontSize: 'clamp(16px, 4vw, 20px)', fontWeight: 700 }}>Welcome, {user?.first_name} Sir 👋</h2>
